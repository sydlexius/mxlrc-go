...
|---------|--------------|------------|-------|
| `cmd/mxlrcsvc-go/main.go` entry point | Official Go docs: commands go in `cmd/` subdirectories. Required for `go install` to produce correctly-named binary. Separates entry point from logic. | Low | Thin main: parse args, wire dependencies, call `app.Run()`. No business logic in main. |
...
| Updated Makefile / CI / GoReleaser | Build tooling references `mxlrc-go` binary name and flat package. Must update for new `cmd/` path and binary name `mxlrcsvc-go`. | Low | Mechanical: update `go build ./cmd/mxlrcsvc-go`, `.goreleaser.yml` main path, CI workflow paths. |
| Module path rename | `go.mod` says `github.com/fashni/mxlrc-go`. Fork identity requires `sydlexius/mxlrcsvc-go`. All import paths change. | Low | `go mod edit -module github.com/sydlexius/mxlrcsvc-go`, then fix all import statements. |
...
| Dependency injection framework | Wire, dig, fx are for large service graphs. This CLI has 4 packages. Manual construction in `main()` is clearer and more debuggable. | Manual wiring in `cmd/mxlrcsvc-go/main.go`. |
